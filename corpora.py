def dora():
    return [
        [
            '@revisamigrieta ayudame',
            'Puedo ayudarte con información para revisar tu estructura en: http://bit.ly/RevisaMiGrieta, espera mientras alguien te ayuda'
        ],
        [
            'Hola',
            'Soy un chatbot poco inteligente, espera mientras alguien te ayuda, revisa tu estructura en: http://bit.ly/RevisaMiGrieta'
        ],
        [
            'que puedes hacer?',
             'Puedo ayudarte con información para revisar tu estructura en: http://bit.ly/RevisaMiGrieta, espera meintras alguien te ayuda'
        ],
        [
            'quiero reportar una grieta en mi casa',
             'Por favor, entra al siguiente link para registrar la falla: http://bit.ly/rescateMX'
        ],
        [
            'tengo una grieta',
             'Por favor, entra al siguiente link para registrar la falla: http://bit.ly/rescateMX'
        ],
        [
            'grieta',
             'Por favor, entra al siguiente link para registrar la falla: http://bit.ly/rescateMX'
        ],
        [
            'cuales son los albergues?',
             'Puedo ayudarte con la lista de albergues en: http://bit.ly/rescateMX'
        ],
        [
            'donde hay albergues?',
             'Puedo ayudarte con la lista de albergues en: http://bit.ly/rescateMX'
        ],
        [
            'necesito un albergue',
             'Puedo ayudarte con la lista de albergues en: http://bit.ly/rescateMX'
        ],
        [
            'albregues disponibles',
             'Puedo ayudarte con la lista de albergues en: http://bit.ly/rescateMX'
        ],
        [
            'centro de acopio',
             'Puedo darte la lista de centros de acopio en: http://bit.ly/rescateMX'
        ],
        [
            'centros de acopio',
             'Puedo darte la lista de centros de acopio en: http://bit.ly/rescateMX'
        ],
        [
            'donde puedo donar?',
             'Puedo darte la lista de centros de acopio en: http://bit.ly/rescateMX'
        ],
    ]

